...
const tscOutput = fs.readFileSync('tsc_master_output.txt', 'utf16le');
...
for (const [filePath, errors] of Object.entries(updates)) {
  ...
  for (const error of errors) {
    ...
    if (error.fullLine.includes("'id' does not exist in type 'MasterProduk")) {
      lineContent = lineContent.replace(/\bid\s*:/g, 'id_masterProduk:');
    } else if (error.fullLine.includes("Property 'id' does not exist on type") && error.fullLine.includes("id_masterProduk")) {
      lineContent = lineContent.replace(/\.id\b/g, '.id_masterProduk');
    } else {
      // General catch all for id to id_masterProduk if it is the only error on that line
      if (lineContent.includes('id:')) lineContent = lineContent.replace(/\bid\s*:/g, 'id_masterProduk:');
      if (lineContent.includes('.id')) lineContent = lineContent.replace(/\.id\b/g, '.id_masterProduk');
    }
